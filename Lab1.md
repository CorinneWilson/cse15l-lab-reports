...
- A path to a *file* as an argument
  ...
  - This output is an error. You must follow a `cd` command with an argument that is a path to a directory. However, `en-us.txt` is not a directory, it is a file. Therefore, this command produces the error shown above. 
